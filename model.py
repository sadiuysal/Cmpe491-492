...
class CustomModel(keras.Model):
    ...
    def train_step(self, data ):
        ...
        if cfg.adversarial_attack:
            ...
            with tf.GradientTape() as tape:
                ...
                loss = self.loss(y_pred, adversarial_selection=True, _lambda=cfg._lambda)
    ...
    def replica_step(self, data):
        # Unpack the data. Its structure depends on your model and
        # on what you pass to `fit()`.
        x = data
        x_2N = tf.concat([self.data_augmentation(x), self.data_augmentation(x)], 0)
        if cfg.adversarial_attack:
            adversaries, adv_loss = attacks.get_adversaries_2(self, x=x, target=self.data_augmentation(x), epsilon=cfg.epsilon ,itr_count=cfg.attacks_itr_count,step_size=cfg.attacks_step_size)
            x_3N = tf.concat([x_2N, adversaries], 0)
            #print(adv_loss)
            with tf.GradientTape() as tape:
                y_pred = self(x_3N, training=True)  # Forward pass
                ...
                # (the loss function is configured in `compile()`)
                loss = self.loss(y_pred, adversarial_selection=True, _lambda=cfg._lambda)
                loss = tf.nn.compute_average_loss(loss, global_batch_size= cfg.GLOBAL_BATCH_SIZE)
                #loss = obj_lib.contrastive_Loss(y_pred,adversarial_selection=True,_lambda=cfg._lambda)
        else:
            with tf.GradientTape() as tape:
                y_pred = self(x_2N, training=True)  # Forward pass
